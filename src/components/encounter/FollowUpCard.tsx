  ...
  return (
    <div style={{ background: T.card, borderRadius: 16, padding: "18px 20px", boxShadow: T.shadow, marginTop: 16 }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", borderBottom: `1px solid ${T.line}`, paddingBottom: 10, marginBottom: 13 }}>
        ...
        <div style={{ fontSize: 11.5, color: T.faint }}>you choose who gets reminded</div>
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {suggestions.map((s, i) => (
              <button
                ...
                style={{ textAlign: "left", font: `400 13px/1.45 ${T.sans}`, color: T.body, background: T.accentBg2, border: `1px solid ${T.accentLine}`, borderRadius: 9, padding: "8px 11px", cursor: "pointer" }}
              >
      ...
      <div style={{ padding: "11px 12px", background: T.panelBg, border: `1px solid ${T.line}`, borderRadius: 12 }}>
        <input
          ...
          style={{ width: "100%", boxSizing: "border-box", font: `400 13.5px/1.5 ${T.sans}`, color: T.ink, background: "#fff", border: `1px solid ${T.line}`, borderRadius: 8, padding: "8px 11px" }}
        />
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 9, flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: T.muted }}>
            due
            <input
              type="date"
              value={dueAt}
              onChange={(e) => setDueAt(e.target.value)}
              style={{ font: `400 13px ${T.mono}`, color: T.ink, background: "#fff", border: `1px solid ${T.line}`, borderRadius: 7, padding: "5px 8px" }}
            />
          </label>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <span style={{ fontSize: 12.5, color: T.muted }}>remind:</span>
            {RECIPIENTS.map((r) => {
          ...
          <button
            onClick={create}
            disabled={!canCreate}
            style={{ marginLeft: "auto", font: `600 13px/1 ${T.sans}`, color: "#fff", background: canCreate ? T.accent : T.faint, border: "none", borderRadius: 8, padding: "8px 14px", cursor: canCreate ? "pointer" : "default" }}
          >
            Add follow-up
          </button>
        </div>
        {recipients.length === 0 && (
          <div style={{ marginTop: 7, fontSize: 12, color: T.amberInk }}>Pick at least one recipient.</div>
        )}
      </div>
      ...
      {followUps.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 12 }}>
          {followUps.map((f) => (
            <div key={f.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", border: `1px solid ${T.line}`, borderRadius: 10, fontSize: 13.5, color: T.body }}>
              ...
              {f.status === "pending" ? (
                <button
                  ...
                  disabled={busy}
                  style={{ font: `600 12px/1 ${T.sans}`, color: T.accent, background: T.accentBg, border: `1px solid ${T.accentLine}`, borderRadius: 7, padding: "6px 10px", cursor: "pointer", whiteSpace: "nowrap" }}
                >
                  Send now
                </button>
              ) : (
                <span style={{ font: `600 11.5px/1 ${T.sans}`, color: T.accentInk, background: T.accentBg, borderRadius: 7, padding: "5px 9px", whiteSpace: "nowrap" }}>
                  {f.status}
                </span>
              )}
            </div>
      ...
      {notice && <div style={{ marginTop: 10, fontSize: 12.5, color: T.accentInk, background: T.accentBg2, border: `1px solid ${T.accentLine}`, borderRadius: 9, padding: "8px 11px", lineHeight: 1.5 }}>{notice}</div>}
      {error && <div style={{ marginTop: 10, fontSize: 12.5, color: T.amberInk }}>{error}</div>}
    </div>
  );
}
