    ...
    return (
      ...
      >
        ...
        <button
          ...
          style={{ font: `500 11.5px/1 ${T.sans}`, color: stillFlagged ? T.amberInk : T.accent, background: "none", border: "none", cursor: "pointer", padding: 0 }}
        >
          undo
        </button>
      </div>
  ...
  return (
    <div style={{ padding: "11px 13px", background: T.amberBg, border: `1px solid ${T.amberLine}`, borderRadius: 10 }}>
      <div style={{ display: "flex", gap: 9, alignItems: "flex-start" }}>
        <span style={{ color: T.amberInk, fontWeight: 700, fontSize: 15, lineHeight: 1.2 }}>⚠</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 700, color: T.amberInk, fontSize: 13.5 }}>Dose check · {finding.medication}</div>
          <div style={{ fontSize: 13, color: T.amberInk, lineHeight: 1.5, marginTop: 2 }}>{finding.message}</div>
          {cite && <div style={{ fontSize: 11.5, color: T.amberInk, opacity: 0.75, marginTop: 3 }}>{cite}</div>}
          {reviseOpen ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9, flexWrap: "wrap" }}>
              <input
                ...
                style={{ font: `400 13.5px/1.5 ${T.mono}`, color: T.ink, background: "#fff", border: `1px solid ${T.amberLine}`, borderRadius: 8, padding: "7px 10px", width: 110 }}
              />
              <button
                onClick={saveRevision}
                disabled={checking || doseDraft.trim().length === 0}
                style={{ font: `600 13px/1 ${T.sans}`, color: "#fff", background: doseDraft.trim() ? T.accent : T.faint, border: "none", borderRadius: 8, padding: "8px 13px", cursor: doseDraft.trim() && !checking ? "pointer" : "default" }}
              >
                {checking ? "Checking…" : "Save & re-check"}
              </button>
              <button
                onClick={() => setReviseOpen(false)}
                style={{ font: `600 13px/1 ${T.sans}`, color: T.amberInk, background: "none", border: `1px solid ${T.amberLine}`, borderRadius: 8, padding: "8px 11px", cursor: "pointer" }}
              >
                Cancel
              </button>
              <span style={{ fontSize: 11.5, color: T.amberInk, opacity: 0.8 }}>Your dose, your words — re-checked against the same reference.</span>
            </div>
          ) : (
            <div style={{ display: "flex", gap: 8, marginTop: 9 }}>
              <button
                onClick={() => setReviseOpen(true)}
                style={{ font: `600 13px/1 ${T.sans}`, color: "#fff", background: T.accent, border: "none", borderRadius: 8, padding: "8px 13px", cursor: "pointer" }}
              >
                Revise dose…
              </button>
              <button
                onClick={() => onDecide({ kind: "kept" })}
                style={{ font: `600 13px/1 ${T.sans}`, color: T.amberInk, background: "none", border: `1px solid ${T.amberLine}`, borderRadius: 8, padding: "8px 13px", cursor: "pointer" }}
              >
                Keep as documented
              </button>
            </div>
