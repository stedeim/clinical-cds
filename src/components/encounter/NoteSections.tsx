...
export function sectionLabel(): CSSProperties {
  return { font: `700 11.5px/1 ${T.sans}`, letterSpacing: ".12em", textTransform: "uppercase", color: T.accent };
}

export function CodeChip({ children }: { children: React.ReactNode }) {
  return (
    <span style={{ font: `600 12px/1 ${T.mono}`, color: T.accentInk, background: T.accentBg, borderRadius: 5, padding: "3px 7px" }}>
      {children}
    </span>
  );
}
  ...
  return (
    <span style={spanStyle(span.provenance)} title={spanTitle(span.provenance)}>
      ...
      {span.provenance === "inferred" && (
        <sup style={{ font: `700 10px/1 ${T.mono}`, color: "#9a7a1e", marginLeft: 1 }}>AI</sup>
      )}
    </span>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: `${marginTop ?? 0}px 0 7px` }}>
      ...
      {onEdit && (
        <button
          onClick={onEdit}
          style={{ font: `500 11.5px/1 ${T.sans}`, color: T.accent, background: "none", border: "none", cursor: "pointer", padding: 0 }}
        >
          edit
        </button>
      )}
