...
function DoseChip({ finding }: { finding: DoseFinding }) {
  const cite = finding.citation ? ` [${finding.citation.title} — ${finding.citation.source}]` : "";
  return (
    <span
      title={finding.message + cite}
      style={{ font: `600 10px/1 ${T.sans}`, letterSpacing: ".02em", color: T.amberInk, background: T.amberBg, border: `1px solid ${T.amberLine}`, borderRadius: 6, padding: "3px 7px", cursor: "help", whiteSpace: "nowrap" }}
    >
      ⚠ dose
    </span>
  );
...
export function NoteCard({
  ...
  function buildExportText(): string {
    return serializeNote(note, { examLines, signature, doseFindings });
  }
      ...
      {plan.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13.5, color: T.body }}>
          {plan.map((span, i) => {
            ...
            const flagged = finding && (finding.status === "exceeds" || finding.status === "below_threshold");
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span>
                ...
                {flagged && finding && <DoseChip finding={finding} />}
              </div>
            );
          })}
        </div>
